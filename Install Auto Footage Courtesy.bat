...
:check_debug_mode
powershell -NoProfile -Command "$ErrorActionPreference='Stop'; $names=@(Get-ChildItem -Path 'HKCU:\Software\Adobe' -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -match '^CSXS\.\d+$' } | ForEach-Object { $_.PSChildName }); if (-not $names.Count) { $names=@('CSXS.11') }; $targets=@($names | Sort-Object { [int]($_ -replace '^CSXS\.','') } | ForEach-Object { [PSCustomObject]@{ Name=$_; Path=('HKCU:\Software\Adobe\' + $_) } }); $changed=0; foreach ($target in $targets) { $value=(Get-ItemProperty -Path $target.Path -Name PlayerDebugMode -ErrorAction SilentlyContinue).PlayerDebugMode; if ($value -ne '1') { if (-not (Test-Path $target.Path)) { New-Item -Path 'HKCU:\Software\Adobe' -Name $target.Name -Force | Out-Null }; New-ItemProperty -Path $target.Path -Name PlayerDebugMode -PropertyType String -Value '1' -Force | Out-Null; $changed++ } }; if ($changed -gt 0) { Write-Output ('CEP debug mode enabled for ' + $targets.Count + ' CSXS version(s).') } else { Write-Output ('CEP debug mode checked: already enabled.') }"
if errorlevel 1 (
